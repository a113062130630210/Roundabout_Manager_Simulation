...
bool trajectory::place_on_top(const trajectory& t) {
  if (sub_trajs.empty() || t.sub_trajs.empty()) {
    EXIT("[ERROR] place_on_top list empty");
  }
  if (entry_position != t.entry_position) {
    EXIT("[ERROR] place_on_top position not aligned");
  }
  ...
  while (s_it != sub_trajs.end() || t_it != t.sub_trajs.end()) {
    ...
    if (s_it->conflict_with(*t_it)) {
      ...
      auto [t_star, t_int] = pair;
      if (t_star < 0) return false;
...
bool trajectory::avoid_front(const trajectory& target) {
  ...
  while (!nxt_traj.place_on_top(target) && it >= sub_trajs.begin()) {
    double acc           = it->acc;                         // the acceleration of current sub-trajectory
    double time_ellapsed = it->leave_time - it->entry_time; // total time spent of current sub-trajectory (before modification)
    ...
    double e = it->entry_time + time_ellapsed;
    double m = (b + e) / 2;

    for (int i = 0; i < 25; i++) {
      double t = m - it->entry_time;                                 // total time to travel before deceleration
      double v = it->entry_velocity + acc*t;                         // initial velocity of the deceleration process
      double d = distance_left - (it->entry_velocity*t + acc*t*t/2); // distance left for the deceleration process
...
trajectory& trajectory::push_sub_traj(double time, double acc) {
  double et, ex, ev;

  if (sub_trajs.empty()) {
    if (fabs(time - entry_time) <= 1e-10) return *this;
    if (time < entry_time) {
      EXIT("[RANGE_ERROR] trajectory::push_sub_traj 1");
    }
    ...
    if (fabs(time - st.leave_time) <= 1e-10) return *this;
    if (time < st.leave_time) {
  ...
  if (lx > leave_position + 1e-10) {
    EXIT("[RANGE_ERROR] trajectory::push_sub_traj position out of range");
  }

  leave_time = time;
  leave_velocity = lv;
  sub_trajs.emplace_back(et, time, ex, lx, ev, lv, acc);
